                        ...
                        return (
                            <div key={index} className='w-[100%] h-[10%] border-t border-b'>
                                <div className='w-[100%] h-[80%] flex items-start gap-6 bg-[#51808048] py-[10px] px-[20px] rounded-2xl relative'>
                                    ...
                                    <input type="number" min={1} defaultValue={item.quantity} className='md:max-w-20 max-w-10 md:px-2 md:py-2 py-[5px] text-[white] text-[18px] font-semibold bg-[#518080b4] absolute md:top-[40%] justify-center items-center md:left-[50%] border-[1px] border-[#9ff9f9] rounded-md text-center' onChange={(e) => e.target.value === ' ' || e.target.value === '0' ? null : updateQuantity(item._id, item.size, Number(e.target.value))} />

                                    <RiDeleteBin5Line className='text-[#9ff9f9] w-[25px] h-[25px] absolute top-[54%] md:top-[54%] md:right-[5%] right-1 -translate-y-1/2 cursor-pointer' onClick={() => updateQuantity(item._id, item.size, 0)} />
                                </div>

                                <div className='flex justify-start items-end my-20'>
                                    <div className='w-full sm:w-[450px]'>
                                        <CartTotal itemId={item._id} size={item.size} />
                                        <button className='text-[18px] hover:bg-slate-500 cursor-pointer bg-[#51808048] py-[10px] px-[50px] rounded-2xl text-white flex items-center justify-center gap-[20px] border-[1px] border-[#80808049] ml-[30px] mt-[20px]'
                                            onClick={() =>
                                                navigate('/placeorder', { state: { productId: item._id, size: item.size } })
                                            }
                                        >
                                            PROCEED TO CHECKOUT
                                        </button>
